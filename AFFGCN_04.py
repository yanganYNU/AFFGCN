...
if __name__ == "__main__":
    print("Reading data...")
    all_data = read_and_generate_dataset(tf_filename,
                                         wx_filename,
                                         num_of_index,
                                         merge)
    ...
    net = model(c_in=num_tf_features, c_out=64, num_wx_features=38, num_for_predict=12,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for train_tf, train_wx, train_t_wx, train_t_tf, in train_loader:
            train_tf = train_tf.to(device)
            train_wx = train_wx.to(device)
            train_t_wx = train_t_wx.to(device)
            train_t_tf = train_t_tf.to(device)
            net.train()  # train pattern
            optimizer.zero_grad()  # grad to 0

            output, _, _, out_wx = net(train_tf, train_wx, supports)
            l_tf = loss_function(output, train_t_tf)
            l_wx = loss_function(out_wx, train_t_wx)
